#!/bin/sh
HAS_DEB=$(command -v dpkg)
HAS_RPM=$(command -v rpm)
HAS_APT=$(command -v apt)
HAS_SYSCTL=$(command -v systemctl)

#be safe from rpm installed on debian, it will not work for rpm install, only for rpm build!
if [ ! -z $HAS_APT ]; then
	unset HAS_RPM
fi
...
if [ "$ServiceRemoveSucceed" = false ]; then
	...
	if [ -f "/etc/audit/rules.d/CynetAu.rules" ]; then
		echo "removing plugin file /etc/audit/rules.d/CynetAu.rules"
	...
	if [ -f "/etc/audit/plugins.d/CynetAu.conf" ]; then
		...
		restart_AuditD=true
	elif [ -f "/etc/audisp/plugins.d/CynetAu.conf" ]; then
...
#  Remove old files
if [ -f "/opt/Cynet/CynetEPS" ]; then
	echo "Removing /opt/Cynet/CynetEPS"
	rm -f "/opt/Cynet/CynetEPS"
fi

if [ -f "/opt/Cynet/DefaultEpsConfig.ini" ]; then
	echo "Removing /opt/Cynet/DefaultEpsConfig.ini"
	rm -f "/opt/Cynet/DefaultEpsConfig.ini"
fi

if [ -f "/opt/Cynet/cyservice" ]; then
	echo "Removing /opt/Cynet/cyservice"
	rm -f "/opt/Cynet/cyservice"
fi

if [ -f "/opt/Cynet/cyservice.service" ]; then
	echo "Removing /opt/Cynet/cyservice.service"
	rm -f "/opt/Cynet/cyservice.service"
fi
...
if [ -d "/opt/Cynet/Data/" ]; then
	echo "Removing /opt/Cynet/Data/ folder"
	rm -rf "/opt/Cynet/Data/"
fi

if [ -d "/opt/Cynet/AV/" ]; then
	echo "Removing /opt/Cynet/AV/ folder"
	rm -rf "/opt/Cynet/AV/"
fi

#echo "Verify that no CynetEPS instance is running after uninstall"
#echo "Printing ps aux | grep CyntetEPS:"
#ps aux | grep -i CynetEPS
#echo "Printing ps aux | grep CyntetEPS | grep -v sudo:"
#ps aux | grep  -i CynetEPS | grep -v sudo
